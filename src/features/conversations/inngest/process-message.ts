...
export const processMessage = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      // Handle Google API 403 Permission Denied (Invalid API Key)
      if (error instanceof Error && (error.message.includes("403") || error.message.includes("PERMISSION_DENIED") || error.message.includes("unregistered callers"))) {
        throw new NonRetriableError("Google Generative AI API Key is invalid or missing permissions. Please check your GOOGLE_GENERATIVE_AI_API_KEY in .env.local.");
      }
      throw error;
    }
